...
const clientId = `zibase_${Math.random().toString(16).slice(3)}`;
...
function publishOnlineStatus(isOnline) {
    ...
    mqttclient.publish(onlineTopic, message, {
        retain: true
    }, (err) => {
...
mqttclient.on('message', (topic, payload) => {
    ...
    if (S(device).contains('RTS433_')) { // Is it a cover?
...
function publish_hass(id, msg) {
    if (mqttclient.connected == true) {
        logger.debug(msg)
        mqttclient.publish(config.mqtt_topic_hass + '/events/' + id, msg, {
            retain: true,
            qos: 1
        }, (err) => {
            if (err) logger.error(err)
...
function processMessage(msg, rinfo) {
    ...
    if (S(msg).contains('SCENARIO')) {
        ...
        if (position > -1)
            str_hass += `, "position": ${position}`;
        if (brightness)
            str_hass += `, "brightness": ${brightness}`;
        ...
        if (probes[id]) {
            ...
            tags.forEach((tag) => {
                ...
                if (value) {
                    if (!isNaN(value)) { // test if value is a float or int
                        try {
